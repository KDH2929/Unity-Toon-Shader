Shader "Unlit/PrewittFilterOutline"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float Gx = (
                    depthUpLeft * 1 + depthUp * 1 + depthUpRight * 1 +
                    depthLeft * 0 + depth * 0 + depthRight * 0 +
                    depthDownLeft * -1 + depthDown * -1 + depthDownRight * -1
                 );


                float Gy = (
                    depthUpLeft * -1 + depthUp * 0 + depthUpRight * 1 +
                    depthLeft * -1 + depth * 0 + depthRight * 1 +
                    depthDownLeft * -1 + depthDown * 0 + depthDownRight * 1
                 );

                // sqrt(Gx * Gx + Gy * Gy) ≈ abs(Gx) + abs(Gy)     Manhatten 거리로 근사도 가능
