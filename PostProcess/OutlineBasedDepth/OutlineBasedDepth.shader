Shader "Unlit/OutlineBasedDepth"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // 상하좌우 고려
                float Outline = (depthUp - depth) + (depthDown - depth) + (depthLeft - depth) + (depthRight - depth);

                // 대각선 방향 고려
                // Outline += ((depthUpLeft - depth) + (depthDownLeft - depth) + (depthUpRight - depth) + (depthDownRight - depth));
